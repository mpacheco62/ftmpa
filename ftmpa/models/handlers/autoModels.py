...
class AutoElasticity(AutoModels):
    ...
    def calc(self, param: dict[str, float], stress_ref: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        ...
        """
        #  add derivative for lankfords calc

        stress_ref = np.repeat(stress_ref[..., np.newaxis, :, :], repeats=self.npoints, axis=-3)
        multiplier = np.linspace(start=0.0, stop=self.stress_max, num=self.npoints)
        stress = stress_ref*multiplier
...
class AutoAnisotropicAsociated(AutoModels):
    """Automatic generation of pair stress-strain states for anisotropir model  
